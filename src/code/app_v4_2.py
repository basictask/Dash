...
indicators = {
    ...
    "NY.GDP.PCAP.CD": "Egy főre jutó GDP (US$)",
}
...
def update_wb_data():
    ...
    df = pd.merge(df, countries, on="country", how="inner", validate="many_to_many")
...
app.layout = html.Div([
    ...
    html.Div(style={'height': '20px'}),
    # K-közép klaszterezés hozzáadása az alkalmazáshoz
    html.Br(),
    html.Hr(),
    html.Br(),
    dbc.Row([
        dbc.Col([
            html.H2('K-Közép klaszterezés indikátorok szerint', style={'textAlign': 'center'}),
...
@app.callback(
    ...
    Input('indicator_dropdown_kmeans', 'value')
)
def clustered_map(year, n_clusters, indicator):
    """
    Tematikus térkép létrehozása K-közép klaszterezéssel
    """
    ...
    if df.isna().all().any():
        fig = make_empty_fig()
        fig.update_layout(title='No available data for the selected combination of year / indicator.')
        return fig
